    ...
    <div>
        <v-modal v-ref:output :options="options">
            ...
            <div class="uk-modal-header uk-flex uk-flex-middle">
                <h2>{{ 'Importing' | trans }}</h2>
            </div>
            <pre class="pk-pre uk-text-break" v-html="output"></pre>
            <v-loader v-show="status == 'loading'"></v-loader>
            <div class="uk-alert uk-alert-success" v-show="status == 'success'">{{ 'Successfully imported.' | trans }}</div>
...
module.exports = {
	...
	methods: {
		...
		import: function (file, onClose) {
			...
			return this.$http.post ('admin/wordpressimport/import', {
				file: file,
			}, null, {xhr: this.init ()}).then (function () {
				// if (this.status === 'success') {
				//     let index = _.findIndex(packages, 'name', pkg.name);
				//
				//     if (-1 !== index) {
